...
include ("modelo/mcarma.php");
	...
	if (!$sede){
		$sede= isset($_SESSION["sede"]) ? $_SESSION["sede"]:NULL;
	}
...
if (($action == "upload" || $actionS == "upload")) { 
	...
	if($archivo!="carga/"){
		...
		$txm .= "<div class='panel-heading'><h3>Ingreso Masivo de Aprendices</h3></div>";
		...
		while($i<$fin){
			...
			$numero_fila++;
			//Ficha
			$data6 = $ins->selfic($datos[1]);
			//echo $data6[0]["num"];
			if($data6[0]["num"]==0){
				$ins->insfic($datos[1],$datos[2],'4',$sede);
			}
			//Aprendiz
			$data5 = $ins->selapre($datos[3]);
			//echo $data5[0]["num"];
			if($data5[0]["num"]==0){
				$ins->insapre($datos[3],$datos[4],$datos[1],rand(100000, 999999),'0');
			}else{
				$ins->updapre($datos[3],$datos[4],$datos[1]);
			}
		}
		$np=$np+1;
		if($action){
		...
		$_SESSION['np']=$np;
		echo "<script language='Javascript'>location.href='home.php?pg=1099';</script>";
	}
}
?>